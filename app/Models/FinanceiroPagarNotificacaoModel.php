 ...
 * @since      10/04/2021
 */
class FinanceiroPagarNotificacaoModel extends NajModel {
    ...
    public function getAllUsersWithFinancialPayerNotification() {
        $Sysconfig  = new SysConfigModel();
        $daysNotify = $Sysconfig->searchSysConfig('FINANCEIRO', 'NOTIFICA_CLIENTE_APP_PAGAR_DIAS');
        ...
        foreach($daysNotify as $day) {
            ...
            $allPerson = DB::select("
                ...
                AND (
                    CONTA.TIPO = 'R' 
                    AND (CONTA.PAGADOR='1' or CONTA.PAGADOR is null)
                )
