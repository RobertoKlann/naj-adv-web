...
use App\Models\FinanceiroPagarNotificacaoModel;
 ...
 * @since      10/04/2021
 */
class FinanceiroPagarNotificacaoController extends NajController {

    protected $actionNotification = '@ACT/open_to_pay';

    public function onLoad() {
        $this->setModel(new FinanceiroPagarNotificacaoModel);
    }
    ...
    private function sendManyNotificationOneSignal() {
        ...
        $users = $this->getModel()->getAllUsersWithFinancialPayerNotification();
        $erros = [];
    ...
    private function getMessagePush($user) {
        ...
        if($user->days == date('d/m/Y'))
            return "Olá {$user->pessoa_nome}, você possui uma parcela com vencimento em {$user->days} (vence hoje) no valor de R$ {$payment} {$user->descricao}. (Parcela: {$user->parcela_atual} de {$user->parcela_total}).";

        if($user->days == date('d/m/Y', strtotime("+1 day")))
            $message = "Olá {$user->pessoa_nome}, você possui uma parcela com vencimento em {$user->days} (vence amanhã) no valor de R$ {$payment} {$user->descricao}. (Parcela: {$user->parcela_atual} de {$user->parcela_total}).";

        if($user->days == date('d/m/Y', strtotime("+2 day")))
            $message = "Olá {$user->pessoa_nome}, você possui uma parcela com vencimento em {$user->days} (vence em 2 dias) no valor de R$ {$payment} {$user->descricao}. (Parcela: {$user->parcela_atual} de {$user->parcela_total}).";

        // if($user->days == date('d/m/Y', strtotime("+3 day")))
            $message = "Olá {$user->pessoa_nome}, você possui uma parcela com vencimento em {$user->days} (vence em 3 dias) no valor de R$ {$payment} {$user->descricao}. (Parcela: {$user->parcela_atual} de {$user->parcela_total}).";
    ...
    private function validateDateHourNotificationSended() {
        $Sysconfig = new SysConfigModel();
        $dateHour  = $Sysconfig->searchSysConfig('FINANCEIRO', 'NOTIFICA_CLIENTE_APP_PAGAR');
